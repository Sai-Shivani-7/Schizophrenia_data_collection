...
# ─────────────────────────────────────────────────────────────
# JS BROWSER TIMER  — zero dependency on Python reruns
# ─────────────────────────────────────────────────────────────
...
def js_timer(seconds: int, uid: str):
    ...
    components.html(f"""
    <div id="tmr_{uid}" class="mq-timer idle" style="color: white;">
  🎤 Click the mic below — timer starts when you do
    ...
    (function(){{
      ...
      const paint=()=>{{
        ...
        (left>HALF?"active":left>DANGER?"warning":"danger");
      }};
      ...
      const start=()=>{{
        ...
        iv=setInterval(()=>{{
          ...
          if(left<=0){{
            clearInterval(iv);
            DIV.textContent="⏰ TIME'S UP — click stop";
            DIV.className="mq-timer danger";
          }}else{{
            paint();
          }}
        }},1000);
...
def upload_audio(audio_bytes, label, pid):
    ts   = datetime.now().strftime("%Y%m%d_%H%M%S")
    name = f"{pid}_{label}_{ts}.wav"
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
        tmp.write(audio_bytes); path = tmp.name
    try:
        media = MediaFileUpload(path, mimetype='audio/wav')
...
def try_upload(audio, label):
    """Upload once; return True when done."""
    if audio and label not in st.session_state.uploads:
        with st.spinner("Saving audio…"):
            st.session_state.uploads[label] = upload_audio(audio.getvalue(), label, st.session_state.pid)
        st.rerun()
...
# ═══════════════════════════
    ...
    <div style="text-align:center;margin:1.5rem 0;padding:1rem;background:rgba(57,255,20,0.05);border-radius:12px;border:1px solid rgba(57,255,20,0.2);">
        <div style="font-family:Orbitron;color:#39ff14;font-size:0.9rem;letter-spacing:2px;">✅ ALL AUDIO SECURELY UPLOADED</div>
        <div style="color:#8888bb;font-size:0.85rem;margin-top:0.4rem;">Thank you! Your data is anonymous and encrypted.</div>
    </div>
